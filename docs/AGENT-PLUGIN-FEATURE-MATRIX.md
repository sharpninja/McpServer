...
| **Offline Cache / Failsafe Replay** | Yes (cache/pending + flush on reconnect) | Yes (special handoff + repaired behavior) | Yes (.mcpServer/failsafe/cline) | Yes (.mcpServer/failsafe/cline-v2) | Yes (cache + failsafe replay) | Yes | Yes (full) | Yes (.mcpServer/failsafe/opencode) |
