...
2. Define an `IWorkspaceApiClient` interface in `McpServer.UI.Core` and implement it in `McpServer.Director` (injected at DI registration time)
3. Have `McpServer.UI.Core` reference `McpServer.Client` which already has `WorkspaceClient.ListAsync()`
