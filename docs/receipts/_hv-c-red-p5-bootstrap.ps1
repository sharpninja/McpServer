...
$sessionId = "GrokSubagentHostile-$utc-c-red-p5"
$requestId = "req-$utc-001-hostile-c-red-p5-fixture"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile review PLAN-PLUGINHANDOFF-001 C-red-P5 isolated server fixture'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'CLASS 1 hostile review of PLAN-PLUGINHANDOFF-001 Phase C-red-P5 only. Surfaces A+B+C+D. Do not require P5 green. PLAN stays not done. Prior C-green-P4 AGREE docs/receipts/hostile-validator-20260822T003817Z.md. Re-run PluginIntegrationServerFixtureTests. Read fixture source. Live todo_get. Attack fixture already starts a real host; tests skipped; P5 green mixed in; 7147 reused.'
    model = 'grok-build-subagent'
...
Invoke-Plugin -Method 'client.SessionLog.AppendDialogAsync' -Params @{
    ...
    requestId = $requestId
    items = @(
        @{ timestamp = $now; role = 'model'; category = 'observation'; content = 'add-profile executed first. 18 non-skill profile markdown files read. Work class 1. Surfaces A+B+C+D apply. Scope is C-red-P5 only after C-green-P4 AGREE. Starting independent PluginIntegrationServerFixtureTests rerun, fixture source read, live todo_get, 7147/skip/green-mix attacks.' }
        @{ timestamp = $now; role = 'model'; category = 'decision'; content = 'Decision: treat this as CLASS 1 project implementation red-phase gate for isolated server fixture tests. Consequence: AGREE only if the six named P5 tests exist, fail (not skip), StartAsync throws not implemented, stub does not use developer 7147 DB, tests are new vs P4 greens, and PLAN/MCP-PLUGININT remain done false. Alternatives: scoring P6 green or full PLAN DoD. Rejected: this brief forbids requiring P5 green and forbids marking PLAN done.' }
    )
