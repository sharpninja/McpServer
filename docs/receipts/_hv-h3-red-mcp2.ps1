...
$ids = Get-Content -LiteralPath 'F:\GitHub\McpServer\docs\receipts\_hv-h3-red-ids.txt'
...
[void](Invoke-McpRpc -Method 'initialize' -Params @{
    ...
    capabilities = @{}
    clientInfo = @{ name = 'hostile-validator-h3-red-2'; version = '1.0.0' }
})
...
foreach ($kind in @('fr', 'tr', 'test', 'mapping')) {
    ...
    foreach ($item in $items) {
        ...
        if ($blob -match 'PRODUCT') {
            $clip = $blob
            if ($clip.Length -gt 700) { $clip = $clip.Substring(0, 700) }
