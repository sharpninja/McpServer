...
try {
    ...
    [ordered]@{ nonceSent = $nonce; nonceEcho = $health.nonce; status = $health.status; match = ($health.nonce -eq $nonce); storage = $health.storage; version = $health.version; pid = $health.pid } |
...
try {
    $svc = Get-Service -Name 'McpServer' -ErrorAction Stop
    $cim = Get-CimInstance -ClassName Win32_Service -Filter "Name='McpServer'" -ErrorAction SilentlyContinue
    $proc = $null
    if ($cim -and $cim.ProcessId -gt 0) {
        $proc = Get-Process -Id ([int]$cim.ProcessId) -ErrorAction SilentlyContinue
    }
    $listen = @(Get-NetTCPConnection -LocalPort 7147 -State Listen -ErrorAction SilentlyContinue | Select-Object LocalAddress, LocalPort, OwningProcess)
    $dotnetOn7147 = @()
    foreach ($row in $listen) {
        if ($row.OwningProcess) {
            $p = Get-Process -Id $row.OwningProcess -ErrorAction SilentlyContinue
            if ($p) {
                $dotnetOn7147 += [ordered]@{
                    pid = $p.Id
    ...
    [ordered]@{
        ...
        cimPid = if ($cim) { [int]$cim.ProcessId } else { $null }
        cimPath = if ($cim) { [string]$cim.PathName } else { $null }
        cimStartMode = if ($cim) { [string]$cim.StartMode } else { $null }
        processName = if ($proc) { $proc.ProcessName } else { $null }
        processStartUtc = if ($proc) { try { $proc.StartTime.ToUniversalTime().ToString('o') } catch { $null } } else { $null }
        claimedPid6220Alive = [bool](Get-Process -Id 6220 -ErrorAction SilentlyContinue)
        pid6220Name = if (Get-Process -Id 6220 -ErrorAction SilentlyContinue) { (Get-Process -Id 6220).ProcessName } else { $null }
        listeners = $listen
        listenerProcessCount = $dotnetOn7147.Count
        listenerProcesses = $dotnetOn7147
        programDataExists = Test-Path -LiteralPath 'C:\ProgramData\McpServer'
    } | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $out 'service-pid.json') -Encoding utf8
    Write-Output ("SERVICE=" + $svc.Status + " PID=" + $(if ($cim) { $cim.ProcessId } else { 'none' }))
} catch {
    Save-Text (Join-Path $out 'service-pid.json') $_.Exception.ToString()
    Write-Output 'SERVICE_PID_FAIL'
}

$agent = 'GrokSubagentHostile'
$sessionId = "GrokSubagentHostile-$utc-c-green-p20"
$requestId = "req-$utc-001-hostile-c-green-p20"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile review PLAN-PLUGINHANDOFF-001 C-green-P20 UpdateService harness'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'CLASS 1 hostile C-green-P20. Surfaces A+B+C+D. Review only. Do not mark PLAN-PLUGINHANDOFF-001 or MCP-PLUGININT-001 done. Do not start Phase D. Attack C-green-P20 after C-red-P20 AGREE 20260822T132413Z: named tests green, Development UpdateService sanitized harness, Staging/Production promotion blocked without operator approval.'
    model = 'grok-build-subagent'
...
Invoke-Plugin -Method 'client.SessionLog.AppendDialogAsync' -Params @{
    ...
    requestId = $requestId
    items = @(
        @{ timestamp = $now; role = 'model'; category = 'observation'; content = 'add-profile executed first. 18 non-skill profile markdown files read under C:\Users\kingd\.claude\profile (excluded add-profile.grok.md). Work class 1. Surfaces A+B+C+D apply. Scope is C-green-P20 only. Independent re-run of PluginUpdateServiceHarnessTests, live todo_get, live TEST-MCP-PLUGININT-001, promotion product files, pluginint-p20 receipt, prior 132413Z AGREE confirmation.' }
        @{ timestamp = $now; role = 'model'; category = 'decision'; content = 'Decision: treat this as CLASS 1 C-green-P20 phase-gate. Consequence: AGREE only if 132413Z is independently AGREE, reds existed before this green, independent named-filter is Failed 0 Passed 2 Skipped 0, harness receipt is Development/UpdateService/sanitizedFixtures with no raw apiKey, promotion gate exists and Staging/Production require approval, planted approval artifact is absent, PLAN/PLUGININT remain done false, Phase D not started. Alternatives rejected: treating implementer TRX as proof without re-run; marking PLAN/PLUGININT done; mixing Phase D.' }
    )
...
Invoke-Plugin -Method 'workflow.sessionlog.queryHistory' -Params @{ agent = 'P20Harness'; limit = 10 } -Name 'sl-p20harness-history'
Invoke-Plugin -Method 'client.SessionLog.QueryAsync' -Params @{ agent = 'P20Harness'; limit = 10 } -Name 'sl-p20harness-query'
Invoke-Plugin -Method 'client.Tools.SearchAsync' -Params @{ keyword = 'mcpserver-grok-plugin' } -Name 'tools-search-grok'
