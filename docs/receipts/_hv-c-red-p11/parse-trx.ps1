...
function Get-TrxSummary {
    ...
    $notImpl = @($outcomes | Where-Object { $_.message -match 'PluginSessionLogWorkflowAdapter\.ExecuteCanonicalTurnAsync is not implemented\.' })
    $hostKinds = @('Codex', 'ClaudeCode', 'ClaudeCowork', 'Copilot', 'Grok', 'Cline', 'ClineV2', 'OpenCode')
    $p11ByHost = foreach ($h in $hostKinds) {
        $rows = @($p11 | Where-Object { $_.name -match [regex]::Escape($h) })
        [ordered]@{
            HostKind = $h
            Count = $rows.Count
            Failed = @($rows | Where-Object { $_.outcome -eq 'Failed' }).Count
            Passed = @($rows | Where-Object { $_.outcome -eq 'Passed' }).Count
            Skipped = @($rows | Where-Object { $_.outcome -in @('NotExecuted', 'Skipped', 'Inconclusive') }).Count
            NotImplementedMessage = @($rows | Where-Object { $_.message -match 'PluginSessionLogWorkflowAdapter\.ExecuteCanonicalTurnAsync is not implemented\.' }).Count
        }
    }
    [ordered]@{
        exists = $true
        ...
        p11Skipped = @($p11 | Where-Object { $_.outcome -in @('NotExecuted','Skipped','Inconclusive') }).Count
        p12Count = $p12.Count
        p13Count = $p13.Count
        notImplementedCount = $notImpl.Count
        p11ByHost = $p11ByHost
        unitOutcomes = $outcomes
    } | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $dest -Encoding utf8
}

Get-TrxSummary -TrxPath (Join-Path $out 'dotnet-p11-filter.trx') -Name 'filter'
...
$filterLog = Join-Path $out 'dotnet-p11-filter.log'
