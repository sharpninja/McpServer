...
function Get-TrxSummary {
    ...
    [ordered]@{
        ...
        p12Count = $p12.Count
        p12Failed = @($p12 | Where-Object { $_.outcome -eq 'Failed' }).Count
        p12Passed = @($p12 | Where-Object { $_.outcome -eq 'Passed' }).Count
        p12Skipped = @($p12 | Where-Object { $_.outcome -in @('NotExecuted','Skipped','Inconclusive') }).Count
        p13Count = $p13.Count
        p13Failed = @($p13 | Where-Object { $_.outcome -eq 'Failed' }).Count
        p13Passed = @($p13 | Where-Object { $_.outcome -eq 'Passed' }).Count
        p13Skipped = @($p13 | Where-Object { $_.outcome -in @('NotExecuted','Skipped','Inconclusive') }).Count
        p14Count = $p14.Count
        notImplementedCount = $notImpl.Count
        p11ByHost = @(Summarize-ByHost $p11)
        p12ByHost = @(Summarize-ByHost $p12)
        p13ByHost = @(Summarize-ByHost $p13)
