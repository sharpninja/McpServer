...
$idsPath = 'F:\GitHub\McpServer\docs\receipts\_hv-h5-done-ids.txt'
if (-not (Test-Path -LiteralPath $idsPath)) {
    throw 'ids file missing; run collect first'
}
...
$init = Invoke-McpRpc -Method 'initialize' -Params @{
    ...
    capabilities = @{}
    clientInfo = @{ name = 'hostile-validator-h5-done-1'; version = '1.0.0' }
}
Save-Body -Name '_hv-h5-done-init.json' -Result $init
...
Write-Output ('PRODUCT_TOOLS_HTTP=' + ($productTools -join ','))
Write-Output ('PRODUCT_TOOLS_HTTP_COUNT=' + $productTools.Count)
$names | Set-Content -LiteralPath (Join-Path $outDir '_hv-h5-done-tool-names.txt') -Encoding utf8

$todo = Invoke-McpTool -Name 'todo_get' -Arguments @{
    id = 'MCP-PRODUCTS-001'
    workspacePath = $workspace
}
Save-Body -Name '_hv-h5-done-todo.json' -Result $todo
...
foreach ($kind in @('fr', 'tr', 'test', 'mapping')) {
    ...
    Save-Body -Name ('_hv-h5-done-req-' + $kind + '.json') -Result $res
    ...
    foreach ($item in $items) {
        ...
        if ($id -match 'PRODUCT' -or $blob -match 'PRODUCT') {
            $clip = $blob
            if ($clip.Length -gt 1600) { $clip = $clip.Substring(0, 1600) }
...
$begin = Invoke-McpTool -Name 'sessionlog_begin_turn' -Arguments @{
    ...
    queryText = 'Hostile validator H5-done: attack MCP-PRODUCTS-001 done claim. Class 1. Do not mark TODO. Implementer claims all five FRs, CQRS-only, isolation, DoD, full Test 0/0, ValidateTraceability Succeeded. TODO stays Done=false until AGREE.'
}
