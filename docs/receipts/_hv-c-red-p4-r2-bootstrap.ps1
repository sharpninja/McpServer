...
try {
    ...
    $exact = $false
    if ($search.items) { $exact = @($search.items | Where-Object { $_.name -eq 'mcpserver-grok-plugin' }).Count -gt 0 }
    elseif ($search) { $exact = @($search | Where-Object { $_.name -eq 'mcpserver-grok-plugin' }).Count -gt 0 }
...
$sessionId = "GrokSubagentHostile-$utc-c-red-p4-r2"
$requestId = "req-$utc-001-hostile-c-red-p4-rereview"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile re-review PLAN-PLUGINHANDOFF-001 C-red-P4 after DISAGREE'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'CLASS 1 re-review of PLAN-PLUGINHANDOFF-001 Phase C-red-P4 after DISAGREE docs/receipts/hostile-validator-20260821T235922Z.md. Surfaces A+B+C+D. Review only. Re-attack C3 TR AC1 pinning and D5 trivial JSON deserialize. Re-run CatalogTests. Live todo_get. PLAN stays not done.'
    model = 'grok-build-subagent'
...
Invoke-Plugin -Method 'client.SessionLog.AppendDialogAsync' -Params @{
    ...
    requestId = $requestId
    items = @(
        @{ timestamp = $now; role = 'model'; category = 'observation'; content = 'add-profile executed first. 18 non-skill profile markdown files read. Work class 1. Surfaces A+B+C+D apply. Prior receipt DISAGREE C3 and D5. Starting independent CatalogTests rerun, catalog type/JSON/test read, live todo_get, and sibling entrypoint probe using catalog-declared paths.' }
    )
