...
Get-TrxSummary -TrxPath (Join-Path $out 'trx-p14-hv\p14-filter.trx') -Name 'hv-filter'
Get-TrxSummary -TrxPath (Join-Path $out 'trx-all-hv\pluginint-all.trx') -Name 'hv-all'
Get-ConsoleCounts -LogPath (Join-Path $out 'hv-dotnet-p14-filter.log') -Dest (Join-Path $out 'hv-dotnet-p14-filter-console.json')
...
if (Test-Path -LiteralPath $listLog) {
    ...
    } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath (Join-Path $out 'hv-list-tests-summary.json') -Encoding utf8
}
...
Extract-TodoDone -Path (Join-Path $out 'hv-live-todo-plan.txt') -Dest (Join-Path $out 'hv-live-todo-plan-done.json')
Extract-TodoDone -Path (Join-Path $out 'hv-live-todo-pluginint.txt') -Dest (Join-Path $out 'hv-live-todo-pluginint-done.json')
Extract-TodoDone -Path (Join-Path $out 'hv-live-todo-plan-client.txt') -Dest (Join-Path $out 'hv-live-todo-plan-client-done.json')
Extract-TodoDone -Path (Join-Path $out 'hv-live-todo-pluginint-client.txt') -Dest (Join-Path $out 'hv-live-todo-pluginint-client-done.json')

Write-Output 'HV_PARSE_DONE'
