...
$filterArgs = @(
    ...
    '--filter', 'FullyQualifiedName~PluginHostProcessAdapterTests',
...
$sw.Stop()
[ordered]@{
    Command = 'dotnet test tests/McpServer.PluginIntegration.Tests -c Debug --filter FullyQualifiedName~PluginHostProcessAdapterTests'
    ExitCode = $filterExit
