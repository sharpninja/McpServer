...
$sigObj | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $out 'marker-sig.json') -Encoding utf8
Write-Output ("SIG_MATCH=$sigMatch")

# Health nonce
$nonce = [guid]::NewGuid().ToString('N')
try {
    $health = Invoke-RestMethod -Uri ("http://PAYTON-LEGION2:7147/health?nonce=$nonce") -Method Get -TimeoutSec 15
    $healthObj = [ordered]@{ nonceSent = $nonce; nonceEcho = $health.nonce; status = $health.status; match = ($health.nonce -eq $nonce); storage = $health.storage }
...
# Tool registry (bootstrap exception)
...
try {
    ...
    elseif ($search) { $names = @($search | ForEach-Object { $_.name }) }
...
$sessionId = "GrokSubagentHostile-$utc-c-green-p1-p3"
$requestId = "req-$utc-001-hostile-c-green-p1-p3"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile validate PLAN-PLUGINHANDOFF-001 C-green-P1-P3'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'Independently re-verify PLAN-PLUGINHANDOFF-001 Phase C-green-P1-P3 claims for PluginSessionLogIntegration target, PluginIntegration project, eight-row catalog, P1 tests green, C-red-P1 AGREE ordering, and live TODO done flags.'
    model = 'grok-build-subagent'
