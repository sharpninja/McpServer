...
$scratch = Join-Path ([System.IO.Path]::GetTempPath()) ('hv-s2-hgreen-flush-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    Write-Output ("WROTE {0} unres={1} okGone={2} okExit={3} failExit={4} failOut={5} failClaim={6}" -f $out, $obj.UnresolvedExit0Empty, $obj.OkPendingGone, $ok.ExitCode, $fail.ExitCode, $fail.Stdout, $obj.FailMatchesClaim)
} finally {
