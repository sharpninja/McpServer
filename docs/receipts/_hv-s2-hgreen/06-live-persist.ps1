...
$obj = [ordered]@{
    ...
    HangCmdPath = $hang
    Degraded = $(if ($details) { $details['degraded'] } else { $null })
    Queued = $(if ($details) { $details['queued'] } else { $null })
}
$obj | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $out -Encoding utf8
Write-Output ("WROTE {0} threw={1} persisted={2} elapsed={3} hangCmd={4} degraded={5} queued={6}" -f $out, $threw, $persisted, $obj.ElapsedSec, $obj.WhichReplIsHangCmd, $obj.Degraded, $obj.Queued)
