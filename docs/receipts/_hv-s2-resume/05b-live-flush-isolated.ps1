...
$out = Join-Path $outDir '05b-live-flush-isolated.json'
$lib = Join-Path $wt 'plugins\core\lib-ps'
$hook = Join-Path $lib 'plugin-hook.ps1'
$scratch = Join-Path $outDir 'scratch-flush-isolated'
...
$parentEnv = [ordered]@{
    MCP_WORKSPACE_PATH = [string]$env:MCP_WORKSPACE_PATH
    MCPSERVER_WORKSPACE_PATH = [string]$env:MCPSERVER_WORKSPACE_PATH
    MCP_WORKSPACE_START_DIR = [string]$env:MCP_WORKSPACE_START_DIR
    CLAUDE_PROJECT_DIR = [string]$env:CLAUDE_PROJECT_DIR
    MCP_CACHE_DIR_OVERRIDE = [string]$env:MCP_CACHE_DIR_OVERRIDE
    MCP_PLUGIN_ROOT = [string]$env:MCP_PLUGIN_ROOT
}
...
$ok = Invoke-HookChild -WorkingDirectory $okCwd -Environment $commonOk
...
$fail = Invoke-HookChild -WorkingDirectory $failCwd -Environment $commonFail
...
Write-Output ("WROTE {0} okGone={1} okExit={2} okOut={3} failExit={4} failOut={5} failGonePending={6}" -f $out, $okPendingGone, $ok.ExitCode, $ok.Stdout, $fail.ExitCode, $fail.Stdout, (-not $failPendingStillThere))
