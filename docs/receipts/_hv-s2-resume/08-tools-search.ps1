...
$out = Join-Path $outDir '08-tools-search.json'
...
if ($null -ne $resp) {
    ...
    elseif ($resp.PSObject.Properties.Name -contains 'tools') { $items = @($resp.tools) }
...
Write-Output ("WROTE {0} items={1} exact={2} err={3}" -f $out, $obj.ItemCount, $obj.ExactNameCount, $err)
