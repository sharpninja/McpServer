...
$init = Invoke-McpRpc -Method 'initialize' -Params @{
    ...
    capabilities = @{}
    clientInfo = @{ name = 'hostile-validator-h5-rerun-1'; version = '1.0.0' }
}
Save-Body -Name '_hv-h5-rerun-init.json' -Result $init
...
Write-Output ('HAS_SESSIONLOG_REPLACE=' + ($names -contains 'sessionlog_replace_section'))
...
$names | Set-Content -LiteralPath (Join-Path $outDir '_hv-h5-rerun-tool-names.txt') -Encoding utf8

$todo = Invoke-McpTool -Name 'todo_get' -Arguments @{
    id = 'MCP-PRODUCTS-001'
    workspacePath = $workspace
}
Save-Body -Name '_hv-h5-rerun-todo.json' -Result $todo
...
foreach ($kind in @('fr', 'tr', 'test', 'mapping')) {
    ...
    Save-Body -Name ('_hv-h5-rerun-req-' + $kind + '.json') -Result $res
    ...
    foreach ($item in $items) {
        ...
        if ($id -match 'PRODUCT' -or $blob -match 'PRODUCT') {
            $clip = $blob
            if ($clip.Length -gt 1800) { $clip = $clip.Substring(0, 1800) }
