...
# Replay Codex live, Codex quarantine, and Claude quarantine SubmitAsync records.
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

$pluginRoot = 'F:\GitHub\mcpserver-grok-plugin'
$workspace = 'F:\GitHub\McpServer'
$receiptPath = Join-Path $workspace 'docs\receipts\failsafe-other-replay-20260821T150540Z.json'
$repl = Join-Path $pluginRoot 'lib\repl-invoke.ps1'
$dirs = @(
    (Join-Path $workspace '.mcpServer\codex\failsafe'),
    (Join-Path $workspace '.mcpServer\codex\failsafe\quarantine'),
    (Join-Path $workspace '.mcpServer\claude\failsafe\quarantine')
)
...
$files = foreach ($dir in $dirs) {
    if (Test-Path -LiteralPath $dir -PathType Container) {
        Get-ChildItem -LiteralPath $dir -Filter '*.yaml' -File | Sort-Object FullName
    }
}
...
foreach ($file in $files) {
    ...
    try {
        ...
        $params = $null
        if ($doc -is [System.Collections.IDictionary] -and $doc.Contains('params')) {
            $params = $doc['params']
        }
        if ($null -eq $params) { throw 'record has no params' }
...
Write-Output ("posted={0} failed={1}" -f $posted, $failed)
