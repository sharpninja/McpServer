...
function Get-TrxSummary {
    ...
    $p8 = @($outcomes | Where-Object { $_.name -match 'Adapter_CodexAndPowerShellHook_UseDeclaredEntrypoint|Adapter_ClineV1_StdioSessionLogTools|Adapter_ClineV2AndOpenCode_UseProductionExport' })
    [ordered]@{
        exists = $true
        path = $TrxPath
        ...
        p7Skipped = @($p7 | Where-Object { $_.outcome -in @('NotExecuted','Skipped','Inconclusive') }).Count
        p7NotImplementedCount = @($p7 | Where-Object { $_.message -match 'PluginHostProcessAdapter\.LaunchAsync is not implemented' }).Count
        p8Count = $p8.Count
