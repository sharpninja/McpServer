...
try {
    ...
    $exact = $false
    if ($search.PSObject.Properties.Name -contains 'items' -and $null -ne $search.items) {
...
$sessionId = "GrokSubagentHostile-$utc-c-green-p4"
$requestId = "req-$utc-001-hostile-c-green-p4-catalog"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile review PLAN-PLUGINHANDOFF-001 C-green-P4 catalog validation'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'CLASS 1 hostile review of PLAN-PLUGINHANDOFF-001 Phase C-green-P4 only. Surfaces A+B+C+D. Do not require P5-P20. PLAN stays not done. Prior C-red-P4 AGREE docs/receipts/hostile-validator-20260822T001811Z.md. Re-run PluginIntegration.Tests. Read LoadAndValidate. Live todo_get. Attack still throws not implemented, tests skipped, TR AC1 ignored, uniqueness still repository name, PLAN done.'
    model = 'grok-build-subagent'
...
Invoke-Plugin -Method 'client.SessionLog.AppendDialogAsync' -Params @{
    ...
    requestId = $requestId
    items = @(
        @{ timestamp = $now; role = 'model'; category = 'observation'; content = 'add-profile executed first. 18 non-skill profile markdown files read. Work class 1. Surfaces A+B+C+D apply. Scope is C-green-P4 only after C-red-P4 AGREE. Starting independent PluginIntegration.Tests rerun, LoadAndValidate read, live todo_get, sibling entrypoint probe, and fail-closed probes.' }
    )
