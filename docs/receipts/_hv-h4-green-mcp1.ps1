...
$idsPath = 'F:\GitHub\McpServer\docs\receipts\_hv-h4-green-ids.txt'
...
$init = Invoke-McpRpc -Method 'initialize' -Params @{
    ...
    capabilities = @{}
    clientInfo = @{ name = 'hostile-validator-h4-green-1'; version = '1.0.0' }
}
Save-Body -Name '_hv-h4-green-init.json' -Result $init
...
$names | Set-Content -LiteralPath (Join-Path $outDir '_hv-h4-green-tool-names.txt') -Encoding utf8

$todo = Invoke-McpTool -Name 'todo_get' -Arguments @{
    id = 'MCP-PRODUCTS-001'
    workspacePath = $workspace
}
Save-Body -Name '_hv-h4-green-todo.json' -Result $todo
...
foreach ($kind in @('fr', 'tr', 'test', 'mapping')) {
    ...
    Save-Body -Name ('_hv-h4-green-req-' + $kind + '.json') -Result $res
...
$open = Invoke-McpTool -Name 'sessionlog_open' -Arguments @{
    ...
    sessionId = $sessionId
    workspacePath = $workspace
    title = 'Hostile H4-green products context review'
    model = 'grok'
}
Save-Body -Name '_hv-h4-green-open.json' -Result $open
...
Save-Body -Name '_hv-h4-green-begin.json' -Result $begin
