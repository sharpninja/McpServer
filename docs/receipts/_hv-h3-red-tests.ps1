...
function Invoke-NamedTest {
    ...
    $out = Join-Path $outDir ('_hv-h3-red-' + $Name + '.txt')
...
function Invoke-ListTests {
    ...
    $out = Join-Path $outDir ('_hv-h3-red-list-' + $Name + '.txt')
    $start = [datetime]::UtcNow
    @(
        'NAME=' + $Name
        'PROJECT=' + $Project
...
Invoke-ListTests -Name 'ctrl' -Project 'tests/McpServer.Support.Mcp.Tests' -Filter 'FullyQualifiedName~ProductsControllerTests'
Invoke-NamedTest -Name 'ctrl' -Project 'tests/McpServer.Support.Mcp.Tests' -Filter 'FullyQualifiedName~ProductsControllerTests'
...
Write-Output 'H3_RED_TESTS_DONE'
