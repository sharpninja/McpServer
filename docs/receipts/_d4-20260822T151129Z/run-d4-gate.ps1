...
$outDir = 'F:\GitHub\McpServer\docs\receipts\_d4-20260822T151129Z'
[void][System.IO.Directory]::CreateDirectory($outDir)

function Write-ProgressLine {
    param([string]$Text)
    $line = ('{0} {1}' -f [DateTimeOffset]::UtcNow.ToString('yyyy-MM-ddTHH:mm:ssZ'), $Text)
    Add-Content -LiteralPath (Join-Path $outDir 'live-progress.txt') -Value $line -Encoding utf8
    Write-Output $line
}
    ...
    try {
        ...
        foreach ($r in $results) {
            ...
            if ($outcome -eq 'Failed') {
                $failedNames.Add([string]$r.testName) | Out-Null
            }
        }
        return [ordered]@{
            total        = $total
...
function Get-LockingProcesses {
    param([string]$Pattern, [switch]$ExcludeIntegration)
    $hits = @()
    foreach ($p in @(Get-CimInstance Win32_Process | Where-Object { $_.CommandLine })) {
        $cl = [string]$p.CommandLine
        if ($cl -notmatch $Pattern) { continue }
        if ($ExcludeIntegration -and $cl -match 'IntegrationTests') { continue }
        $hits += [pscustomobject]@{ ProcessId = $p.ProcessId; Name = $p.Name; CommandLine = $cl }
    }
    return $hits
}

function Wait-ForLocks {
    param(
    ...
    while ($true) {
        $hits = @(Get-LockingProcesses -Pattern $Pattern -ExcludeIntegration:$ExcludeIntegration)
        if ($hits.Count -eq 0) { return }
        if ([DateTime]::UtcNow -ge $deadline) {
            $detail = ($hits | ForEach-Object { 'PID={0} {1}' -f $_.ProcessId, $_.Name }) -join '; '
            throw ('Timeout waiting for lock ({0}): {1}' -f $Reason, $detail)
        }
        Write-ProgressLine ('WAIT reason={0} locks={1}' -f $Reason, $hits.Count)
...
Write-ProgressLine 'START D4 remainder full 9-command gate'
...
$commands = @(
    [ordered]@{
        ...
        waitPattern = 'McpServer\.Client\.Tests'
        waitExcludeIntegration = $false
        waitRemainder = $false
    }
    [ordered]@{
        ...
        waitPattern = 'McpServer\.Support\.Mcp\.Tests'
        waitExcludeIntegration = $true
        waitRemainder = $false
    }
    [ordered]@{
        ...
        waitPattern = 'McpServer\.Repl\.Core\.Tests'
        waitExcludeIntegration = $false
        waitRemainder = $false
    }
    [ordered]@{
        ...
        waitPattern = 'McpServer\.Support\.Mcp\.IntegrationTests'
        waitExcludeIntegration = $false
        waitRemainder = $false
    }
    [ordered]@{
        ...
        waitPattern = 'McpServer\.Repl\.IntegrationTests'
        waitExcludeIntegration = $false
        waitRemainder = $false
    }
    [ordered]@{
...
foreach ($c in $commands) {
    if (-not [string]::IsNullOrWhiteSpace([string]$c.waitPattern)) {
        $exclude = [bool]$c.waitExcludeIntegration
        Wait-ForLocks -Reason ('command {0} {1}' -f $c.index, $c.name) -Pattern $c.waitPattern -ExcludeIntegration:$exclude
    }
    if ([bool]$c.waitRemainder) {
        Wait-ForLocks -Reason ('remainder Test before {0}' -f $c.name) -Pattern 'receipts\\_d4-remainder\.ps1'
        Wait-ForLocks -Reason ('McpServer testhost before {0}' -f $c.name) -Pattern 'McpServer\\tests\\'
        Wait-ForLocks -Reason ('McpServer build.ps1 Test before {0}' -f $c.name) -Pattern 'McpServer\\build\.ps1 Test'
    }

    $logPath = Join-Path $outDir ('{0:00}-{1}.log' -f $c.index, $c.name)
    $started = [DateTimeOffset]::UtcNow
    Write-ProgressLine ('START index={0} name={1}' -f $c.index, $c.name)
    ...
    Write-ProgressLine ('END index={0} exit={1} passed={2} failed={3} skipped={4}' -f $c.index, $rec.exitCode, $rec.passed, $rec.failed, $rec.skipped)
    ...
    if ($rec.gateFailed -or $rec.gateSkipped -or $rec.gateExitFail) {
        ...
        Write-ProgressLine ('STOP ' + $stoppedReason)
        break
    }
}
...
$summary = [ordered]@{
    ...
    agent         = 'GrokCode'
    sessionId     = 'GrokCode-20260822T151608Z-pluginhandoff-d4-remainder'
    requestId     = 'req-20260822T151608Z-001-pluginhandoff-d4-remainder'
    ...
    overall       = [ordered]@{
        ...
        allGreen    = [bool]($allGreen -and $overallFailed -eq 0 -and $overallSkipped -eq 0 -and $results.Count -eq 9)
...
Write-ProgressLine ('OVERALL_FAILED=' + $overallFailed)
Write-ProgressLine ('OVERALL_SKIPPED=' + $overallSkipped)
Write-ProgressLine ('ALL_GREEN=' + $summary.overall.allGreen)
Write-ProgressLine ('STOPPED=' + $stoppedReason)
exit $(if ($summary.overall.allGreen) { 0 } else { 1 })
