...
$prodFiles = Get-ChildItem -Path $productsDir -Recurse -File -Filter *.cs

$ctxHits = @($prodFiles | Select-String -Pattern 'ContextDocument|ContextChunk|db\.Documents|db\.Chunks')
...
$regexHits = @($prodFiles | Select-String -Pattern 'PROD-\[A-Z\]\[A-Z0-9\]')
...
Write-Output ('PRODUCTS_CTRL_EXISTS=' + (Test-Path $ctrl) + ' LW=' + (Get-Item $ctrl).LastWriteTimeUtc.ToString('o'))
Write-Output ('STDIO_PRODUCTS_EXISTS=' + (Test-Path $stdio) + ' LW=' + (Get-Item $stdio).LastWriteTimeUtc.ToString('o'))
...
$tools | ForEach-Object { Write-Output ('STDIO_TOOL=' + $_.LineNumber + ':' + $_.Line.Trim()) }

$scopeHits = @(Select-String -Path $reqCtrl -Pattern 'productScope')
Write-Output ('REQ_PRODUCTSCOPE_HITS=' + $scopeHits.Count)
$scopeHits | ForEach-Object { Write-Output ('SCOPE=' + $_.LineNumber + ':' + $_.Line.Trim()) }

$packHits = @(Select-String -Path $ctxCtrl -Pattern 'product-requirements|GetProductRequirementContext|productChunks')
...
$lockHits = @(Select-String -Path $handoff -Pattern 'CreateAsync|check-and-add|TryAdd|already exists|Duplicate')
Write-Output ('HANDOFF_LOCK_HITS=' + $lockHits.Count)
$lockHits | ForEach-Object { Write-Output ('HANDOFF_LOCK=' + $_.LineNumber + ':' + $_.Line.Trim()) }
...
foreach ($t in $tracking) {
    ...
    $create = @(Select-String -Path $t.FullName -Pattern 'CreateAsync|TryAdd|ContainsKey|already exists|lock')
    $create | ForEach-Object { Write-Output ('TRACK_HIT=' + $_.LineNumber + ':' + $_.Line.Trim()) }
}
...
foreach ($f in @($implTest, $implTrace)) {
    if (Test-Path $f) {
        ...
        Get-Content -LiteralPath $f -Tail 25 | ForEach-Object { Write-Output ('IMPL_TAIL=' + $_) }
    } else {
...
# Named AC tests
$testRoot = Join-Path $workspace 'tests\McpServer.Support.Mcp.Tests'
$named = @(
    'CreateProduct',
    'InvalidKey',
    'Duplicate',
...
$allTestCs = Get-ChildItem -Path $testRoot -Recurse -Filter *Product*.cs
foreach ($n in $named) {
    $hits = @($allTestCs | Select-String -Pattern $n -SimpleMatch)
    Write-Output ('NAMED=' + $n + ' HITS=' + $hits.Count)
}

Write-Output 'GREPS2_DONE'
