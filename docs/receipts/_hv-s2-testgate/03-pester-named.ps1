...
$obj = [ordered]@{
    ...
    ExitCodeImplied = $(if ($result.FailedCount -eq 0 -and $result.SkippedCount -eq 0) { 0 } else { 1 })
...
$summary = "WROTE $out Passed=$($result.PassedCount) Failed=$($result.FailedCount) Skipped=$($result.SkippedCount) Total=$($result.TotalCount) NotRun=$($result.NotRunCount) Result=$($result.Result) Elapsed=$($obj.ElapsedSec)"
