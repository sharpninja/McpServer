...
$total = $null; $failed = $null; $passed = $null; $skipped = $null
if ($console -match '(?m)Failed!\s+Failed:\s+(\d+),\s+Passed:\s+(\d+),\s+Skipped:\s+(\d+),\s+Total:\s+(\d+)') {
    ...
    if ($console -match '(?m)^Failed:\s+(\d+)\s*$') { $failed = [int]$Matches[1] }
    if ($console -match '(?m)^Passed:\s+(\d+)\s*$') { $passed = [int]$Matches[1] }
    if ($console -match '(?m)^Skipped:\s+(\d+)\s*$') { $skipped = [int]$Matches[1] }
    if ($console -match '(?m)^Total:\s+(\d+)\s*$') { $total = [int]$Matches[1] }
}
