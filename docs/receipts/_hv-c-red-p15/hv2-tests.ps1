...
$transcript = Join-Path $out 'tests-run.transcript.log'
...
& dotnet test tests/McpServer.PluginIntegration.Tests -c Debug --list-tests --results-directory (Join-Path $out 'list-results') *>&1 | Tee-Object -FilePath $listLog
...
$filterTrxDir = Join-Path $out 'trx-p15'
...
$filterLog = Join-Path $out 'dotnet-p15-filter.log'
...
[ordered]@{
    ...
    FinishedUtc = [DateTime]::UtcNow.ToString('o')
} | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $out 'dotnet-p15-filter-exit.json') -Encoding utf8
Write-Output ("FILTER_EXIT=$filterExit")
Write-Output ("END_UTC=" + [DateTime]::UtcNow.ToString('o'))
Write-Output 'HV2_TESTS_DONE'
try { Stop-Transcript | Out-Null } catch { }
