...
Get-Content -LiteralPath (Join-Path $out 'todo-plan-final-done.json') -Raw
Write-Output '---'
Get-Content -LiteralPath (Join-Path $out 'todo-pluginint-final-done.json') -Raw
Get-Content -LiteralPath (Join-Path $out 'todo-yaml-git-status-after.json') -Raw
