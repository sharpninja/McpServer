...
. 'F:\GitHub\mcpserver-grok-plugin\lib\yaml-object-mutation.ps1'
...
$turn = [ordered]@{
    ...
    auditFiles = 0
    auditCommits = 0
    queryText = 'CLASS 1 hostile review of PLAN-PLUGINHANDOFF-001 Phase C-red-P16.'
    ...
    markerLastWriteUtc = '2026-08-21T10:20:37.4844112Z'
}
$state = [ordered]@{
    status = 'verified'
    agent = $agent
...
$turnYaml = ConvertTo-Yaml -Data $turn -Options WithIndentedSequences
$stateYaml = ConvertTo-Yaml -Data $state -Options WithIndentedSequences
Set-Content -LiteralPath (Join-Path $cacheRoot 'current-turn.yaml') -Value $turnYaml -Encoding utf8
Set-Content -LiteralPath (Join-Path $cacheRoot 'session-state.yaml') -Value $stateYaml -Encoding utf8
Write-Output 'SEEDED_CACHE'
...
} -Name 'sl-complete-seeded'

try {
    $cmdOut = & $plugin -Command CompleteTurn -Response 'Hostile C-red-P16 review AGREE. Receipt docs/receipts/hostile-validator-20260822T073737Z.md.' -WorkspacePath 'F:\GitHub\McpServer' -CacheRoot $cacheRoot -TimeoutSeconds 120 *>&1
    Set-Content -LiteralPath (Join-Path $out 'sl-complete-cmd-seeded.txt') -Value (($cmdOut | ForEach-Object { "$_" }) -join "`n") -Encoding utf8
    Write-Output 'OK sl-complete-cmd-seeded'
} catch {
    Set-Content -LiteralPath (Join-Path $out 'sl-complete-cmd-seeded.txt') -Value $_.Exception.ToString() -Encoding utf8
    Write-Output 'FAIL sl-complete-cmd-seeded'
}
...
Invoke-Plugin -Method 'client.SessionLog.QueryAsync' -Params @{
    ...
    sessionId = $sessionId
    limit = 20
} -Name 'sl-query-sid-seeded'

Write-Output 'SEED_AND_COMPLETE_DONE'
