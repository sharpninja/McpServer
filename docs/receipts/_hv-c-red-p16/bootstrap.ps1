...
$sessionId = "GrokSubagentHostile-$utc-pluginhandoff-p16red"
$requestId = "req-$utc-001-hostile-c-red-p16-aitheory"
...
Invoke-Plugin -Method 'client.SessionLog.OpenSessionAsync' -Params @{
    agent = $agent
    sessionId = $sessionId
    title = 'Hostile review PLAN-PLUGINHANDOFF-001 C-red-P16 AiTheory companion'
...
Invoke-Plugin -Method 'client.SessionLog.BeginTurnAsync' -Params @{
    ...
    queryText = 'CLASS 1 hostile C-red-P16 after C-green-P15 AGREE 20260822T071202Z. Surfaces A+B+C+D. Review only. Do not mark TODOs done. Do not implement P17-P18.'
    model = 'grok-build-subagent'
...
Invoke-Plugin -Method 'client.SessionLog.AppendDialogAsync' -Params @{
    ...
    requestId = $requestId
    items = @(
        @{ timestamp = $now; role = 'model'; category = 'observation'; content = 'add-profile executed first. 18 non-skill profile markdown files read under C:\Users\kingd\.claude\profile (excluded add-profile.grok.md). Work class 1. Surfaces A+B+C+D apply. Scope is C-red-P16 after C-green-P15 AGREE docs/receipts/hostile-validator-20260822T071202Z.md. Independent AiTheory_Agent_RequiresValidJsonFields filter plus live todo_get and PLUGININT FR/TR/TEST.' }
        @{ timestamp = $now; role = 'model'; category = 'decision'; content = 'Decision: treat this as CLASS 1 project implementation red-phase gate for P16 only. Consequence: AGREE only if the named 8-row AiTheory filter is Failed 8 Passed 0 Skipped 0, P17 names absent, EvaluateAsync throws not implemented, PLAN/MCP-PLUGININT remain done false. Alternatives: requiring P17-P18 green or plan closeout. Rejected: parent forbids those.' }
    )
...
Invoke-Plugin -Method 'client.SessionLog.AppendActionsAsync' -Params @{
    ...
    requestId = $requestId
    actions = @(
        @{ order = 1; description = 'add-profile executed; 18 non-skill profile files read'; type = 'read'; status = 'completed'; filePath = 'C:\Users\kingd\.claude\profile' }
        @{ order = 2; description = 'Test-MarkerSignature and health nonce bootstrap'; type = 'tool_call'; status = 'completed'; filePath = 'F:\GitHub\McpServer\AGENTS-README-FIRST.yaml' }
        @{ order = 3; description = 'CLASS 1 C-red-P16 only; do not mark PLAN or PLUGININT done'; type = 'design_decision'; status = 'completed'; filePath = 'docs/plans/PLAN-PLUGINHANDOFF-001.md' }
    )
