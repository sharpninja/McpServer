...
function Get-TrxSummary {
    ...
    $p16ByHost = foreach ($h in $hostKinds) {
        $pat = 'hostKind:\s*' + [regex]::Escape($h) + '\)'
        $matched = @($p16 | Where-Object { $_.name -match $pat })
        [ordered]@{
            HostKind = $h
            Count = @($matched).Count
...
$fullTrx = Get-ChildItem -LiteralPath $out -Recurse -Filter 'pluginint-all.trx' -ErrorAction SilentlyContinue | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1
...
if ($fullTrx) { Get-TrxSummary -TrxPath $fullTrx.FullName -Name 'full' } else { Get-TrxSummary -TrxPath (Join-Path $out 'missing-pluginint-all.trx') -Name 'full' }
...
if (Test-Path -LiteralPath $listLog) {
    ...
    $hostKinds = @('Codex', 'ClaudeCode', 'ClaudeCowork', 'Copilot', 'Grok', 'Cline', 'ClineV2', 'OpenCode')
    $hostCounts = foreach ($h in $hostKinds) {
        $pat = 'AiTheory_Agent_RequiresValidJsonFields\(hostKind: ' + $h + '\)'
        [ordered]@{ HostKind = $h; Count = ([regex]::Matches($listText, [regex]::Escape("AiTheory_Agent_RequiresValidJsonFields(hostKind: $h)"))).Count }
    }
    [ordered]@{
        p16 = ([regex]::Matches($listText, 'AiTheory_Agent_RequiresValidJsonFields')).Count
        p17 = ([regex]::Matches($listText, 'AiTheory_RejectsInvalidJson_DoesNotOverrideDeterministicFailure')).Count
        p18 = ([regex]::Matches($listText, 'NukeTarget_SkipIsFailure')).Count
        p19 = ([regex]::Matches($listText, 'PluginNativeSuite_|PluginInt_P19_')).Count
        hostCounts = @($hostCounts)
        availableHeader = [bool]($listText -match 'The following Tests are available')
