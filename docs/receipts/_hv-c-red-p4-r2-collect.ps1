...
$priorDisagree = 'F:\GitHub\McpServer\docs\receipts\hostile-validator-20260821T235922Z.md'
$priorDisagreeJson = 'F:\GitHub\McpServer\docs\receipts\hostile-validator-20260821T235922Z.json'
...
$priorDisagreeCutoff = [DateTime]::Parse('2026-08-21T23:59:22Z').ToUniversalTime()
...
$meta = foreach ($p in $paths) {
    ...
    if ($exists) {
        ...
        [pscustomobject]@{
            ...
            AfterPriorCRedP4Disagree = ($lw -gt $priorDisagreeCutoff)
        }
    } else {
        [pscustomobject]@{
            Path = $p
...
if (Test-Path -LiteralPath $priorDisagree) {
    $dText = Get-Content -LiteralPath $priorDisagree -Raw
    $dObj = [ordered]@{
        Exists = $true
        Length = (Get-Item -LiteralPath $priorDisagree).Length
        LastWriteTimeUtc = (Get-Item -LiteralPath $priorDisagree).LastWriteTimeUtc.ToString('o')
        OverallVerdictLineMatch = [bool]($dText -match '(?m)^OverallVerdict:\s*DISAGREE\s*$')
        MentionsC3 = ($dText -match 'C3')
        MentionsD5 = ($dText -match 'D5')
    }
    if (Test-Path -LiteralPath $priorDisagreeJson) {
        $dj = Get-Content -LiteralPath $priorDisagreeJson -Raw | ConvertFrom-Json
        $dObj.JsonOverallVerdict = [string]$dj.OverallVerdict
        $dObj.JsonFailCount = $dj.FailCount
        $dObj.ExplicitFailList = @($dj.ExplicitFailList)
    }
    $dObj | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $out 'prior-disagree.json') -Encoding utf8
}
...
if ($jsonScan.Exists) {
    ...
    $jsonScan.HasEnvProperty = ($jsonScan.PropertyNames -contains 'requiredEnvironmentVariables') -or ($jsonScan.PropertyNames -contains 'environmentVariables')
    ...
    $hostKinds = @($probe | ForEach-Object { $_.hostKind })
    $definedKinds = @('Codex','ClaudeCode','ClaudeCowork','Copilot','Grok','Cline','ClineV2','OpenCode')

    $simUnique = (
        (@($probe | Where-Object { [string]::IsNullOrWhiteSpace($_.agentSourceType) -or [string]::IsNullOrWhiteSpace($_.cacheFolder) }).Count -eq 0) -and
        ($uniqueKeyCount -eq $probe.Count)
    )
    $simRoots = (@($probe | Where-Object { -not $_.dirExists }).Count -eq 0)
    $simKinds = (@($probe | Where-Object { $definedKinds -notcontains $_.hostKind }).Count -eq 0)
    $simEntry = (@($probe | Where-Object {
        [string]::IsNullOrWhiteSpace($_.entrypoint) -or $_.entrypointHasDotDot -or (-not $_.catalogEntrypointExists) -or (-not $_.envAllNonEmpty)
    }).Count -eq 0)
    $simVersion = (@($probe | Where-Object { -not ($_.versionExists -or $_.packageJsonExists) }).Count -eq 0)
    $simEight = (($probe.Count -eq 8) -and (@($probe | Where-Object { -not $_.enabled }).Count -eq 0) -and (@($probe | Where-Object { $_.name -eq 'Cline v2' }).Count -ge 1))

    $simulate = [ordered]@{
        UniqueAgentSourceCacheWouldPass = $simUnique
        RepositoryRootsExistWouldPass = $simRoots
        SupportedHostKindsWouldPass = $simKinds
        RequiredEntrypointFilesExistWouldPass = $simEntry
        VersionMetadataPresentWouldPass = $simVersion
        ExactlyEightEnabledWouldPass = $simEight
        AllSixWouldPass = ($simUnique -and $simRoots -and $simKinds -and $simEntry -and $simVersion -and $simEight)
        UniqueKeyCount = $uniqueKeyCount
        RowCount = $probe.Count
        UniqueCacheFolderCount = $cacheUniqueCount
        DistinctHostKinds = @($hostKinds | Select-Object -Unique)
        MissingExpectedNames = $jsonScan.MissingExpectedNames
        DummyReadmeEntrypointWouldPassCount = @($probe | Where-Object {
            Test-Path -LiteralPath (Join-Path (Join-Path $parent $_.repositoryName) 'README.md')
        }).Count
        TestsAssertUniqueCacheFolderAlone = ($testsSrc -match 'CacheFolder[\s\S]{0,80}Distinct' -or $testsSrc -match 'Select\(row => row\.CacheFolder\)')
        TestsAssertAllEightNames = ($testsSrc -match 'Claude Cowork' -and $testsSrc -match 'OpenCode' -and $testsSrc -match 'Codex')
        TestsAssertSpecificEnvNames = ($testsSrc -match 'CODEX_PLUGIN_ROOT' -or $testsSrc -match 'GROK_PLUGIN_ROOT' -or $testsSrc -match 'CLINE_PLUGIN_ROOT')
        TestsAssertSpecificCacheNames = ($testsSrc -match '"cline-v2"' -or $testsSrc -match "'cline-v2'")
        TestsLoadJsonFileDirectly = ($testsSrc -match 'plugin-sessionlog-scenarios\.json')
    }
    $simulate | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $out 'deserialize-simulate.json') -Encoding utf8
}
