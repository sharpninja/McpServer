...
**Version:** v0.1.8
**Status:** Draft. Codex findings from merged PR #56, the PR #58 pin exception, the v0.1.7 hostile review, and the v0.1.8 Astra re-review are incorporated below. Not operator-approved. Proposes a capability beyond the v0.1.x scope of the whitepaper and the proposed implementation; nothing here is approved or scheduled.
**Companion to:** `whitepaper-weighted-context-projection-v0.1.md` (v0.1.11) and `proposed-implementation-weighted-context-projection-v0.1.md` (v0.1.6)
...
**Joint admission is same-generation co-presence.** An admitted edge is a claim about this generation's snapshot (WP §6.1), not a promise that the other endpoint appears later. The endpoints this table requires are both in the assembly for generation G, or the edge admits neither endpoint in G.

- `explains`, `constrains`, and `depends_on`: the load-bearing target and the admitted source both stay. Do not demote the target to pay for the source. If both do not fit, admit neither.
- `shares_root_cause`: both endpoints, or neither.
- `invalidates`: the source, plus the target in the form this table requires (expanded pin with the invalidation marker, or the unpinned marked stub). If that pair does not fit, fail closed under WP §6.2 and WP §6.3. Do not drop the pin, do not omit the marker, and do not admit the source while dropping the target's required form.
...
- `J_void` (provisional **4**) is spent on ordering items 2 and 3 (voided judgments). Unused `J_void` slots MAY be used by stale edges, then by items 1, 4, and 5.
...
- The remainder (`J - J_void - J_stale`, provisional **2**) is for items 1, 4, and 5.
...
Model-call tokens are bounded by `J * T_j`. The cheap eligibility walk is not bounded by `J`. Its bound is §5.4. Deferred candidates share the backlog cap in §5.4.
...
- **Constraint scan.** On append, a turn that records an operator constraint, an environment condition, or a standing "never" commitment (WP §5.1) MUST be marked `constraint-fact`, including when it has no criterion id. If a load-bearing turn has no effective `explains` or `constrains` edge, the generator MUST propose up to `K_constraint` newest `constraint-fact` turns (provisional **`K_constraint = 4`**). The failure text does not have to contain the constraint string. `K_constraint` counts against `K_ref` (`K_constraint ≤ K_ref`). If the mark is missing and the failure shares no referent, record `constraint-scan-empty`. That record is not a successful recovery.

Both paths produce candidates only. Type and direction still require out-of-band judgment (§3, §3.1, §5.1).

**What the caps actually bound.** Eligibility and emission are different limits. The plan structure, including cross-phase criteria, decides which pairs may be considered. The caps decide how many are emitted and how the cheap walk is done. Pairing every turn on one criterion is quadratic. The generator MUST NOT emit all pairs. v0.1.7 said work per generation is O(K) and that revalidation cost is bounded by `J` rather than by graph size. Those two sentences overstated the cheap work and are withdrawn.

The bounds that hold:

- **Emission.** A newly considered turn emits at most `K_crit` criterion-graph hits (provisional **8**): the latest expanded or link-admitted turn on each shared or related criterion, read from a posting list, not every historical sibling. It emits at most `K_ref` referent or constraint-scan hits (provisional **8**).
- **Posting lists.** A criterion id, a referent token, and the `constraint-fact` mark each retain at most `L` turn ids (provisional **`L = 16`**), plus the current load-bearing set on the criterion list. Selection reads those lists. It does not scan every sealed turn each generation.
- **Degree.** A turn holds at most `D` effective edges (provisional **`D = 32`**). A new edge past `D` is deferred, not admitted, until a retraction frees a slot.
...
- **Cheap eligibility is incremental.** Voided edges are enqueued when a seal, evidence, or attribution event is appended, in O(D) for the changed turn. The stale queue is ordered by last-judgment generation, so taking the oldest `J_stale` edges is O(`J_stale`). Per generation this is O(D + K_crit + K_ref + J). A full walk of every live edge every generation is not the algorithm.
- **Backlog.** Unjudged candidates wait in a backlog capped at `M` (provisional **64**). Overflow is deferred and recorded. It is not emitted as an edge, and it is not discarded without a record.

`E_live` is at most `turns * D` under the degree cap. Session model-token cost is O(generations * J * T_j). Session cheap work is O(turns * (D + K_crit + K_ref)). Those are the bounds. The caps MUST be fixed before a linking run can pass or fail.
...
- A turn's **reading** is the attributed change in the scope count and the passing count after a tier-2 or tier-3 revision (the table below). §6.5 applies that reading for the effective verdict only.
...
- **Introduced harm** (scope flat, passing-count down, class `introduced`): `effectiveWeight = min(scorerWeight, 0.20)`, and WP §6.2 applies (marked stub if unpinned; expanded pin plus harm marker if pinned).
...
- **No attributed delta,** or a superseding verdict whose effective class is none of the above: `effectiveWeight = scorerWeight`. Remove the harm annotation. A pin stays expanded. An unpinned turn is no longer forced to the harm stub.
...
- **Adoption gate (predeclared).** Live link admission stays off until a labeled shadow sample passes. There is no live trial mode before the gate: would-be admissions are not copied into the model-facing projection, do not charge the link sub-cap, and are not shown to the actor. The shadow log records, for each proposed edge, whether that edge would have admitted a turn into a recorded shadow assembly under the same snapshot rules as WP §6.1. Admission precision is the fraction of those shadow-admitted edges that an independent labeler marks true. The labeler judges the sealed payloads. The labeler is not a live model that was shown the edge. Shadow usefulness is WP §9.1 metric 5 applied to the shadow-admitted slice: whether the subsequent sealed action would have used the turn correctly, scored by the labeler against the sealed session, not by a model that saw the edge. Provisional, and MUST be fixed before the run can pass or fail: at least 50 proposed edges and at least 30 shadow-admitted edges; admission precision ≥ 0.90 on the shadow-admitted population; shadow usefulness ≥ 0.80. Proposal precision is printed beside the gate and cannot replace it. Below the bar, edges MAY be recorded, revalidated, and shadow-logged, and MUST NOT admit turns (§4). After the bar, live admission MAY turn on. Shadow logging stays.
...
- **Candidate-source share** reports four fractions of proposed edges: requirement-graph traversal, referent match, `constraint-fact` scan, and any other fallback (§5.4). Referent match and the constraint scan are quiet-constraint paths. Neither is a fallback. A falling requirement-graph share with an empty referent-match share and an empty constraint-scan share, or a run that records `constraint-scan-empty`, means the plan anchor is missing work and the constraint marks did not recover it. That record is not a successful recovery.
...
- **No claim that work per generation is O(K), or that revalidation cost is bounded by `J` rather than by graph size.** Both sentences are withdrawn (§5.4). The bounds that hold are the emission caps, posting-list length `L`, degree `D`, the incremental eligibility walk, and `J * T_j` model tokens.
...
| Version | Date | Change |
|---|---|---|
| v0.1.8 | 2026-09-25 | Astra re-review (round 2) against tip `941cfa99`. See the round-2 finding map below. Supersedes the v0.1.7 closures for P1-03, P1-04, P1-05, P1-06, P1-07, and P1-08. Adds joint admission (§3.1), the per-criterion progress vector (§6.1), verdict reversal on `effectiveWeight` (§6.5), reserved stale-edge slots (§5.2), the constraint-fact scan (§5.4), honest cost bounds (§5.4), and a shadow adoption sample (§9). Companions: WP v0.1.11, IMPL v0.1.6 |
...
Full-document gpt-6-astra xhigh re-review of tip `941cfa99`. Verdict on that tip was DISAGREE. P1-01, P1-02, and P1-09 stayed closed and are not reopened. The v0.1.7 rows for P1-03, P1-04, P1-05, P1-06, P1-07, and P1-08 are historical; this section supersedes them. P2-06, P2-07, and P2-08 are the three re-review comments on that tip (hysteresis versus capacity expansion, warning stubs in the fail-closed set, shadow sample for the adoption gate). WP is `whitepaper-weighted-context-projection-v0.1.md`. ADD is this document. IMPL is `proposed-implementation-weighted-context-projection-v0.1.md`.

- **P1-03.** ADD §5.2. Judgment cap `J = 8` is split: `J_void = 4` for seal and evidence voids (items 2 and 3), `J_stale = 2` reserved for the oldest stale edges (item 6; items 1, 4, and 5 MUST NOT consume those slots), remainder 2 for load-bearing, near-threshold, and weight-moved edges. `J_stale < 1` or `J < J_void + J_stale` is non-conforming. At `S + S_starve` (`S = 10`, `S_starve = 10`) a stale edge is promoted ahead of items 4 and 5. Changed evidence and staleness still force judgment. The round-1 skip-rule fix stays.
- **P1-04.** ADD §6.1. Authoritative progress is a per-criterion state vector (`passing`, `failing`, `skipped`, `untested`, `blocked-no-test`), not a single integer. Scope counts every in-scope criterion. Progress counts `passing` only; `skipped` is not passing. A tier-2 run updates only criteria the suite executed. A tier-3 run updates only criteria it executed and MUST NOT replace the vector with a partial count or mark an unrun criterion passing or failing. A criterion id appears once. Effective state is the latest append-only revision. Introduced versus revealed classifies a downward revision of this same vector. WP §5.1 still points here.
- **P1-05.** ADD §6.5. `scorerWeight` is WP §5.2. `effectiveWeight` applies only the latest superseding verdict. Floors and ceilings do not stack. Progress-up and blocker discovery use `max(scorerWeight, 0.60)`. Introduced harm uses `min(scorerWeight, 0.20)` plus WP §6.2. A revealed regression on the revealing turn does not take the harm ceiling. A superseding `revealed` or no-delta verdict removes the 0.20 ceiling and the harm annotation on the next snapshot. Pins stay expanded. An unpinned turn is no longer forced to the harm stub. Pending and actor claims do not revise weight.
- **P1-06.** ADD §5.4 and ADD §1. Exact referent match remains, and it is not sufficient when the failure does not quote the constraint. On append, operator constraints, environment conditions, and "never" commitments MUST be marked `constraint-fact` even with no criterion id. If a load-bearing turn has no effective `explains` or `constrains` edge, propose up to `K_constraint = 4` newest constraint-fact turns, counted inside `K_ref = 8`. A missing mark plus no shared referent records `constraint-scan-empty`, which is not success. Candidates only; type and direction still need out-of-band judgment. Not `similar_to`. §9 reports the constraint scan as its own candidate-source fraction.
- **P1-07.** ADD §5.4 and ADD §11. The sentences "work per generation is O(K)" and "revalidation cost is bounded by `J` rather than by graph size" are withdrawn. Bounds that hold: emission `K_crit = 8` plus `K_ref = 8`; posting lists `L = 16`; degree `D = 32` (overflow deferred, not admitted); model judgments `J` and `J * T_j` (`T_j = 4096`) with the §5.2 slot split; cheap eligibility is incremental O(D + K + J) via a void queue and a stale queue, not a full scan of `E` each generation; `E_live` is at most turns times `D`; session model tokens are O(generations * J * T_j); session cheap work is O(turns * (D + K_crit + K_ref)); backlog `M = 64`.
- **P1-08.** ADD §9 and ADD §4. Live admission stays off until the gate passes. The sample is a shadow population: would-be admissions are logged and do not enter the model-facing projection, do not charge the link sub-cap, and are not shown to the actor. Provisional bar: at least 50 proposed and at least 30 shadow-admitted edges; admission precision ≥ 0.90 on the shadow-admitted population; shadow usefulness ≥ 0.80, labeled against the sealed session rather than by a live model that saw the edge. No live trial before the gate. After the bar, live admission MAY turn on. Shadow logging stays.
- **P1-10.** WP §6.3, restated in IMPL §5.1, Option C, and §8.1. Spill unmarked unpinned content first. Shrink an unpinned harm or invalidation stub to the marker; do not omit the marker. The mandatory fail-closed set includes the prefix, standing memories, pins with their markers, the live result, and every required harm or invalidation marker. Pins stay expanded.
- **P1-11.** ADD §3.1, applied in WP §6.1 step 3. Same-generation co-presence. For `explains`, `constrains`, and `depends_on`, the load-bearing target and the admitted source both stay, or the edge admits neither; do not demote the target to pay for the source. `shares_root_cause` is both or neither. `invalidates` keeps the source plus the target's required form (expanded pin or unpinned marked stub); if they do not fit, fail closed. Do not drop the pin or the marker.
- **P1-12.** WP §6.1 generation snapshot, applied per call in IMPL Option C and §8.1. Generation G freezes seals, effective edges, scorer weights, pins, the progress vector, the turn set, and live content before judgments, admission, or packing. Writes during G become visible at G+1. A reseal, evidence change, or superseding verdict after the freeze aborts G. Each tool-loop model call is its own generation.
- **P2-06.** WP §6 step 5 and WP §6.1 step 6. Packing MUST NOT expand an omitted or summarized turn only because residual capacity grew. Free capacity is not a mandatory transition. Residual budget stays unused unless expansion is already authorized by hysteresis and cooldown or by a mandatory link or harm rule. IMPL §8.1 treats a capacity-only expansion as a fail.
- **P2-07.** Same closure as P1-10. WP §6.3 puts harm and invalidation markers in the mandatory fail-closed set. Spill MUST NOT omit them.
- **P2-08.** Same closure as P1-08. ADD §9 collects the admission-gate sample on a shadow population. The gate does not require live-admitted edges while live admission is off.

### Hostile review remediation (v0.1.7)

Full-document Codex review against tip `acd1453c`. Each finding is closed in the section named here. Round 2 (v0.1.8) supersedes the rows for P1-03, P1-04, P1-05, P1-06, P1-07, and P1-08. P1-01, P1-02, P1-09, P2-01 through P2-05, and P3-01 stay closed as written here. WP is `whitepaper-weighted-context-projection-v0.1.md`. ADD is this document. IMPL is `proposed-implementation-weighted-context-projection-v0.1.md`.
...
- **P1-07 (superseded by the round-2 row).** ADD §5.4. All-pairs inside a criterion is withdrawn. Round 1 also said session work is O(turns times (K_crit + K_ref)) and that `J` bounds revalidation cost. Those cost sentences are withdrawn. Use the round-2 bounds.
