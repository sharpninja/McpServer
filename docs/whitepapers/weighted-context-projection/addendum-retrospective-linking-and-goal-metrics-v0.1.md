...
**Version:** v0.1.6
**Status:** Draft. Codex findings from merged PR #56 and the PR #58 pin exception are incorporated below. Not operator-approved. Proposes a capability beyond the v0.1.x scope of the whitepaper and the proposed implementation; nothing here is approved or scheduled.
...
**Pins stay expanded under `invalidates`.** WP Principle 5 keeps an operator, system, or acceptance pin expanded until it is unpinned. WP §6 step 3 marks every pin `expanded` and, when pins alone exceed `B_turns`, fails closed: surface an error, force memory promotion, or ask the operator. Traversing an `invalidates` edge leaves that pinned payload verbatim in the assembly. The invalidation marker — the result is obsolete, and which edge says so — is attached beside the expanded turn. An unpinned obsolete target that is already present may still be forced to the marked stub in the table. The same pin rule covers every instruction in this section that forces a payload to a stub, including `shares_root_cause`.
