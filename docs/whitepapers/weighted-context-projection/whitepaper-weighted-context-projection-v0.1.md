...
**Pressure inheritance.** A `budget-demotion` stores `pressureSource`: the segment whose growth or new admission consumed the budget, and that segment's `originCause`. Displaced tokens on other turns keep `turnCause` `budget-demotion` and inherit that `originCause` for metric 3. Discretionary summary growth (`summary-rewrite`, or `score` when the growth is a score-driven expand) is a discretionary pressure source. So is a discretionary `edge-score` admission that grows the tail. Pin growth, a live tool result, a mandatory marker, and an invalidation that must fit are mandatory pressure sources. Labeling rewrite-induced displacement as bare `budget-demotion` with no `pressureSource`, or excluding those tokens from metric 3, is non-conforming. Metric 2 still ignores the demotion: only a `projectionState` change whose `turnCause` is `score` enters that numerator. Pins stay expanded. Harm and invalidation stay markers.
...
The §8 labels are measured against the paper's own mechanisms, not against an absent store. Main context holds system instructions, working context (core memory), and a FIFO queue. External context has two persistent stores: recall storage keeps the conversation history outside the window, and archival storage is a database the agent reads and writes. Overflow is a hard context-window ceiling: messages that would exceed it are evicted from the queue into recall storage and replaced in context by a recursive summary, and generation continues.  
Durable full turns is **Partial** because those external stores persist history and documents, while the prompt working set is the FIFO plus recursive summaries, not every stored message. Hard budget is **Soft** because the window is a hard token ceiling and the overflow policy pages and continues. It does not refuse the model call. Soft is that continuation policy. It is not a claim that MemGPT has no limit.  
Misses: weights are not framed as relative task-progress toward completion; the recursive summary of evicted messages is lossy in the prompt, and the paper does not tie re-admit to an append-only session ledger for operator agent work.
...
| MemGPT-style paging | Partial (recall storage + archival database; paged prompt) | Indirect | Via recall | Soft (hard window; pages and continues) | Yes | App-dependent |
...
MemGPT's Partial and Soft cells are the §7.1 reading of Packer et al., arXiv:2310.08560. Partial means the prompt is a working set while recall storage and archival storage persist outside it. Soft means overflow evicts queue messages into recall storage, writes a recursive summary, and continues the call. The context window itself is a hard ceiling.
