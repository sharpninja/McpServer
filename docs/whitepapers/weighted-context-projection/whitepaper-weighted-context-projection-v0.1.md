...
**Pressure inheritance.** A `budget-demotion` stores `pressureSource` as a list. Each entry is `{sourceId, originCause, tokenDelta}` for one growth or new admission that consumed budget in that packing step. One entry is the single-source case. Displaced tokens on other turns keep `turnCause` `budget-demotion`. Discretionary sources are `summary-rewrite`, `score`, and `edge-score`. Mandatory sources are pin growth, a live tool result, a mandatory marker, and an invalidation that must fit. Let `R` be the previous-tail tokens removed by that demotion. Let `M` be the sum of `tokenDelta` on mandatory entries. Let `D` be the sum of `tokenDelta` on discretionary entries. The mandatory share is `floor(R * M / (M + D))` and goes only to `mandatory-token-count`. The discretionary share is `R` minus that floor and goes to the metric 3 numerator. Any remainder token therefore counts in metric 3. The floor formula applies only when both `M` and `D` are greater than 0. When `D` is 0 and `M` is greater than 0, the mandatory share is all of `R`. When `M` is 0 and `D` is greater than 0, the discretionary share is all of `R`. If `M + D` is 0, the demotion names no pressure and is non-conforming. Assigning all of `R` to the pin, or all of `R` to the summary, is non-conforming when both `M` and `D` are positive. Metric 2 still ignores the demotion: only a `projectionState` change whose `turnCause` is `score` enters that numerator. Pins stay expanded. Harm and invalidation stay markers.
