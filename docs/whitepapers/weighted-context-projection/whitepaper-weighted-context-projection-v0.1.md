...
**Commit point.** G is abortable until the request is handed to the model transport (send). Immediately before send, re-read tombstones, seals, evidence watermarks, and superseding verdicts. If any of those differ from the snapshot, do not send. Abort G and retry as the next generation on a new snapshot. After send, the generation is published. A later tombstone, reseal, evidence change, or superseding verdict does not rewrite the in-flight call. It is visible at the next generation. Each tool-loop model call is its own generation, snapshotted after the previous call's appends are in the log, and each has its own send check. Packing MUST NOT mix edges, weights, or deletion visibility written after the snapshot with payloads from before it.

**Required traces.** Tombstone-before-send: both endpoints are live at freeze, one is tombstoned before send, the send is refused, and the linked content is not issued. Reseal-before-send: same refusal. Concurrent supersession: a verdict that lands before send aborts the send; a verdict that lands after send waits for the next generation. Interrupted generation: the process stops before send, nothing is published, and a cursor already appended (ADD §5.4) remains.
...
The projection trace for a state change MUST name the cause: `pin`, `invalidation`, `introduced-harm`, `score`, `budget-demotion`, or `warning-retirement`. IMPL §8 Phase 1's omit/re-admit trace includes that cause. Evaluators of §9.1 read this field and no other source for mandatory versus discretionary.
...
**Warning retirement.** Rules 3 and 4 and the mandatory set in §6.3 apply to markers whose `warningState` is `active` (ADD §7.4). A repaired, consolidated, plan-retired, or memory-transferred warning leaves the mandatory set. The verdict row stays. Retirement does not unpin, and it does not relabel a true introduced-harm verdict as false. Active markers stay spill-protected. A history of resolved regressions that would not fit in `B` as raw markers MUST still be able to send after those retirements, while any marker that is still active remains in the mandatory set.
...
2. **State-flip rate.** Denominator: turns that were `expanded` or `summarized` on the previous published snapshot (the previous active set), not every turn loaded for the session. If the denominator is 0, do not compute a rate. Record `empty-denominator`. The gate passes for this metric. Numerator: turns in that set whose `projectionState` changed for a discretionary `score` cause. Mandatory causes (`pin`, `invalidation`, `introduced-harm`, `budget-demotion`, `warning-retirement`) are counted in `mandatory-transition-count` and are outside the numerator. A generation whose causes are only mandatory is `mandatory-only` and is outside the mean. Stable omissions are outside the denominator. Window: one published generation. Session figure: the mean of counted per-generation rates. A generation is a phase-boundary flush only when its snapshot records `phaseBoundary: true` and a `phaseId`. Evaluators MUST exclude that generation and MUST NOT exclude any other generation by judgment. Provisional bar: **≤ 0.10** when the denominator is at least 1. Operator-adjustable, and the number MUST be fixed before Phase 4 can pass or fail (IMPL §8). Two evaluators of one trace use these fields and no others.
3. **Active-context token replacement.** Separate from the flip rate. A pass can replace the entire variable tail while flipping few states, or flip many tiny turns while leaving the tokens in place. Window: one published generation. Denominator: tokens in the previous generation's variable tail (expanded payloads, stubs, and link-admitted segments). If that token count is 0, do not compute a rate. Record `empty-tail`. The gate passes for this metric. The stable prefix and standing memories are outside both sides, so a large cached prefix cannot dilute the ratio. Numerator: tokens of that previous tail that are absent from the new tail. Exclude a generation from the mean and from the maximum when it is `mandatory-only` or when `phaseBoundary` is true with a `phaseId`, under the same rule as metric 2. A mandatory pin or marker insertion that is the only cause does not fail the bar; it is reported in `mandatory-transition-count`. If any discretionary omit or expand is also present, the generation is counted, and tokens changed only by mandatory causes on other turns are still outside the numerator. Session figures: the per-generation mean and the per-generation maximum of the counted generations. Provisional bar: **≤ 0.25** on each counted generation. Operator-adjustable, and the number MUST be fixed before Phase 4 can pass or fail. Both bars are required. Passing the flip rate does not pass this metric. Edge-flip counting is ADD §9 and uses the same empty-denominator, mandatory-cause, and phase-boundary rules.
