...
Only turn cause `score` is discretionary for the state-flip and token-replacement numerators. Only edge cause `edge-score` is discretionary for the edge-flip numerator. Every other cause is mandatory and is reported outside that numerator. IMPL §8 Phase 1's omit/re-admit trace includes the turn-layer cause.
