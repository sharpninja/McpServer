...
The server shall support federation roles Standalone, DirectProxy, Hub, and LocalProxy so a hub can act as authoritative source of truth while local proxies forward MCP traffic, register hosted workspaces, queue writes during hub outages, replay operations, receive hub fanout, and surface topology status to agents.
