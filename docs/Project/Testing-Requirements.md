...
- TEST-MCP-007: Given workspace registration, when a workspace is created, then it receives a unique incremented port, its directory scaffold is created, its Kestrel host starts, and a `.mcp-server.yaml` marker file is written to its root.
...
- TEST-MCP-009: Given `Mcp:ApiKey` is configured, when a mutating request lacks `X-Api-Key`, then the server returns 401. When a read endpoint decorated with `[SkipApiKeyAuth]` is called without a key, then it returns 200.
...
- TEST-MCP-013: Given a workspace Kestrel host, when `StartAsync` completes, then `.mcp-server.yaml` exists at the workspace root with the correct port and endpoint paths. When `StopAsync` completes, then the marker file is removed.
