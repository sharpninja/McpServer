...
The terminal-turn audit-evidence gate (a turn moving to a terminal status: completed, failed, closed, or cancelled, MUST carry at least one design decision, action, or commit) is a Quad-Brain ACID requirement (see FR-MCP-136 durable audit/session-log boundaries). It SHALL be enforced ONLY when the parent session SourceType is the ACID hosted-agent source type (`McpHostedAgentDefaults.AcidSourceType` = `McpAcidAgent`). Standard (non-Quad-Brain) agent sessions (e.g. `ClaudeCode`, `Cursor`, `Copilot`) SHALL be allowed to close terminal turns without audit evidence; the gate MUST NOT leak into the standard session-log endpoints.
