...
exports.findMarkerFile = findMarkerFile;
exports.parseMarkerField = parseMarkerField;
exports.verifySignature = verifySignature;
exports.fullBootstrap = fullBootstrap;
...
function findMarkerFile(startDir) {
    ...
    // Check root
    if (fs.existsSync('/' + MARKER_FILENAME))
 ...
 */
function parseMarkerField(markerFile, fieldName) {
...
function parseMarkerNestedField(markerFile, sectionName, fieldName) {
    const lines = fs.readFileSync(markerFile, 'utf8').split('\n');
    let inSection = false;
    for (const line of lines) {
 ...
 */
function buildCanonicalPayload(markerFile) {
    const get = (f) => parseMarkerField(markerFile, f) ?? '';
    const lines = [
 ...
 */
function extractStoredSignature(markerFile) {
    const rawLines = fs.readFileSync(markerFile, 'utf8').split('\n');
 ...
 */
function verifySignature(markerFile) {
    ...
    process.stderr.write(`[marker] Signature mismatch (computed=${computed}, stored=${storedSignature})\n`);
    return false;
}
/**
 ...
 */
async function fullBootstrap(startDir = process.cwd()) {
