...
public sealed partial class TunnelListViewModel : AreaListViewModelBase<TunnelProviderSnapshot>
{
    ...
    public async Task LoadAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            var result = await _dispatcher.QueryAsync(new ListTunnelsQuery(), ct);
    ...
    {
        await _dispatcher.SendAsync(new EnableTunnelCommand(providerName), ct);
        await LoadAsync(ct);
    }
    ...
    {
        await _dispatcher.SendAsync(new DisableTunnelCommand(providerName), ct);
        await LoadAsync(ct);
    }
    ...
    {
        await _dispatcher.SendAsync(new StartTunnelCommand(providerName), ct);
        await LoadAsync(ct);
    }
    ...
    {
        await _dispatcher.SendAsync(new StopTunnelCommand(providerName), ct);
        await LoadAsync(ct);
    }
    ...
    {
        await _dispatcher.SendAsync(new RestartTunnelCommand(providerName), ct);
        await LoadAsync(ct);
    }
}
