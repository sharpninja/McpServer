...
/// <summary>
/// Per-workspace CLI-agent prompt overrides for TODO operations.
...
public sealed class TodoPromptOptions
{
    ...
    /// CLI-agent process. This ensures the CLI finds its cached authentication tokens
    ...
    /// CLI-agent process. Required when running as a Windows service that cannot
    /// access the user's keyring. Null or empty = rely on the CLI's default auth.
    /// </summary>
    public string? GitHubToken { get; set; }

    /// <summary>
    /// Absolute path to the CLI agent executable.
    /// Null or empty = use the <c>AgentCliClientOptions.AgentPath</c> default (<c>cline</c>).
    /// </summary>
