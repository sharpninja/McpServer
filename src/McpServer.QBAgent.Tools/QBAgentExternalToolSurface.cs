...
public static class QBAgentExternalToolSurface
{
        ...
        bool allowGitPush)
    {
        ...
        {
            Tool((Func<string, CancellationToken, Task<Client.Models.RepoFileReadResult>>)files.ReadFileAsync,
                "read_file",
                "Read a workspace file by its repo-relative path. Returns the file content and whether it exists."),
            Tool((Func<string, string, CancellationToken, Task<Client.Models.RepoWriteResult>>)files.WriteFileAsync,
                "write_file",
                "Create or overwrite a workspace file with the full given content. Use edit_file for partial changes."),
            Tool((Func<string?, CancellationToken, Task<Client.Models.RepoListResult>>)files.ListFilesAsync,
                "list_files",
                "List files and directories under a repo-relative path (or the workspace root when omitted)."),
            Tool((Func<string, string, string, bool, int?, CancellationToken, Task<FileEditResult>>)files.EditFileAsync,
                "edit_file",
                "Apply a targeted replacement of a unique oldString with newString in a workspace file; set replaceAll for multiple matches."),
            Tool((Func<string, CancellationToken, Task<PowerShellSessionCommandResult>>)powerShell.RunAsync,
                "run_powershell",
                "Run a PowerShell command in the workspace and return its output and error streams. Primary shell on this host."),
            Tool((Func<string, CancellationToken, Task<BashToolResult>>)bash.RunAsync,
                "run_bash",
                "Run a command with Git Bash if available. Returns available=false when bash is not installed; prefer run_powershell."),
            Tool((Func<string, string?, CancellationToken, Task<GitToolResult>>)git.RunAsync,
                "git",
