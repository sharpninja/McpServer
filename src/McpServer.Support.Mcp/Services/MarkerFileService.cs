...
public static class MarkerFileService
{
    ...
    public const string DefaultPromptTemplate =
        ...
        **Before taking ANY action on a user request, you MUST first POST a session log entry to {{baseUrl}}/mcpserver/sessionlog.**
        ...
        **All session log and TODO operations MUST use the McpSession and McpTodo helper modules.**
        ...
        `/mcpserver/sessionlog` and `/mcpserver/todo` endpoints are NOT permitted.
        ...
        2. Query recent session logs: GET {{baseUrl}}/mcpserver/sessionlog?limit=5
        3. Query current TODOs: GET {{baseUrl}}/mcpserver/todo
        ...
        All `/mcpserver/*` REST endpoints require a per-workspace auth token. Include it with every request:
        ...
        1. GET `{{baseUrl}}/server-startup-utc` and compare response `serverStartedAtUtc` to this marker's `serverStartedAtUtc`.
           If they differ, this marker was generated by an older server instance. Re-read the marker before calling `/mcpserver/*`.
        2. GET `{{baseUrl}}/marker-file-timestamp?repoPath={{workspace.WorkspacePath}}` and compare response `lastWriteTimeUtc` to this marker's `markerWrittenAtUtc`.
           If the endpoint timestamp is newer (or the file is missing), re-read the marker before calling `/mcpserver/*`.
        ...
        - **REST API**: All `/mcpserver/*` endpoints (requires `X-Api-Key` header). Full OpenAPI spec at GET {{baseUrl}}/swagger/v1/swagger.json. Interactive Swagger UI at {{baseUrl}}/swagger.
        ...
        **SessionLog** (POST body to `/mcpserver/sessionlog`):
        ```json
        {
        ...
        **before any session log or TODO operations**:
        ...
        Invoke-RestMethod -Uri "{{baseUrl}}/mcpserver/tools/search?keyword=session" -Headers $headers
        Invoke-RestMethod -Uri "{{baseUrl}}/mcpserver/tools/search?keyword=todo" -Headers $headers
        ...
        **Bash — Full session lifecycle:**
        ...
        | GET | `/mcpserver/todo/{id}/prompt/implement` | Get implementation prompt for a todo |
        | GET | `/mcpserver/todo/{id}/prompt/plan` | Get planning prompt for a todo |
        | GET | `/mcpserver/todo/{id}/prompt/status` | Get status prompt for a todo |
        ...
        **TodoCreateRequest** (POST body to `/mcpserver/todo`):
        ```json
        {
        ...
        **TodoUpdateRequest** (PUT body to `/mcpserver/todo/{id}`):
        ```json
        {
        ...
        **Raw API Reference (for understanding only — do NOT use directly):**
        ...
        # GET  {{baseUrl}}/mcpserver/todo              — list todos
        # POST {{baseUrl}}/mcpserver/todo              — create todo
        # PUT  {{baseUrl}}/mcpserver/todo/{id}         — update todo
        # DELETE {{baseUrl}}/mcpserver/todo/{id}       — delete todo
        ```

        ## Available Capabilities
        - Context Search: POST {{baseUrl}}/mcpserver/context/search — semantic + full-text hybrid search over indexed project documents
        - Context Pack: POST {{baseUrl}}/mcpserver/context/pack — retrieve ordered context chunks for a topic
        - Context Sources: GET {{baseUrl}}/mcpserver/context/sources — list all indexed document sources
        - Todo Management: GET/POST/PUT/DELETE {{baseUrl}}/mcpserver/todo — query, create, update, and delete project tasks
        - Repo Files: GET {{baseUrl}}/mcpserver/repo/file, POST {{baseUrl}}/mcpserver/repo/file, GET {{baseUrl}}/mcpserver/repo/list — read, write, and list repository files
        - GitHub Integration: {{baseUrl}}/mcpserver/gh/issues, {{baseUrl}}/mcpserver/gh/pulls, {{baseUrl}}/mcpserver/gh/labels — issue, PR, and label management
        - Tool Registry:GET {{baseUrl}}/mcpserver/tools/search — discover available tools; GET/POST {{baseUrl}}/mcpserver/tools — manage tool definitions
