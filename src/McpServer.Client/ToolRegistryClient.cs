...
public sealed class ToolRegistryClient : McpClientBase
{
    ...
    public async Task<ToolSearchResult> ListAsync(string? workspace = null, CancellationToken cancellationToken = default)
    {
        ...
        return await GetAsync<ToolSearchResult>($"mcpserver/tools{qs}", cancellationToken);
    }
    ...
    public async Task<ToolSearchResult> SearchAsync(string keyword, string? workspace = null, CancellationToken cancellationToken = default)
    {
        ...
        return await GetAsync<ToolSearchResult>($"mcpserver/tools/search?{string.Join("&", parts)}", cancellationToken);
    }

    /// <summary>Get a tool by ID.</summary>
    public async Task<ToolDto> GetAsync(int id, CancellationToken cancellationToken = default)
    {
        return await GetAsync<ToolDto>($"mcpserver/tools/{id}", cancellationToken);
    }
    ...
    {
        return await PostAsync<ToolMutationResult>("mcpserver/tools", request, cancellationToken);
    }
    ...
    {
        return await PutAsync<ToolMutationResult>($"mcpserver/tools/{id}", request, cancellationToken);
    }
    ...
    {
        return await DeleteAsync<ToolMutationResult>($"mcpserver/tools/{id}", cancellationToken);
    }
    ...
    {
        return await GetAsync<BucketListResult>("mcpserver/tools/buckets", cancellationToken);
    }
    ...
    {
        return await PostAsync<BucketMutationResult>("mcpserver/tools/buckets", request, cancellationToken);
    }
    ...
    public async Task<BucketMutationResult> DeleteBucketAsync(string name, bool uninstallTools = false, CancellationToken cancellationToken = default)
    {
        ...
        return await DeleteAsync<BucketMutationResult>($"mcpserver/tools/buckets/{Uri.EscapeDataString(name)}{qs}", cancellationToken);
    }
    ...
    {
        return await GetAsync<BucketBrowseResult>($"mcpserver/tools/buckets/{Uri.EscapeDataString(name)}/browse", cancellationToken);
    }
    ...
    public async Task<ToolMutationResult> InstallFromBucketAsync(string bucketName, string toolName, string? workspace = null, CancellationToken cancellationToken = default)
    {
        ...
        if (workspace is not null) parts.Add($"workspace={Uri.EscapeDataString(workspace)}");
        return await PostAsync<ToolMutationResult>($"mcpserver/tools/buckets/{Uri.EscapeDataString(bucketName)}/install?{string.Join("&", parts)}", null, cancellationToken);
    }
    ...
    {
        return await PostAsync<BucketSyncResult>($"mcpserver/tools/buckets/{Uri.EscapeDataString(name)}/sync", null, cancellationToken);
    }
}
