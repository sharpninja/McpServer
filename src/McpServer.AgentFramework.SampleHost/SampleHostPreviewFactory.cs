...
internal sealed class SampleHostConsoleApplication : IDisposable
{
    ...
    private async Task ExecutePromptAsync(string input, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException)
        {
            await TryFailTurnAsync(
                ...
                "The interactive AgentFramework sample host canceled the active turn.",
                "The chat turn was canceled.",
        ...
        catch (Exception exception)
        {
            ...
            await TryFailTurnAsync(
                turn.RequestId,
                exception.Message,
                "The interactive AgentFramework sample host failed to complete the chat turn.",
                exception.Message,
                [exception.Message],
                CancellationToken.None).ConfigureAwait(false);
        }
        ...
        try
        {
            await _hostedAgent.SessionLog.CompleteTurnAsync(
                new SessionLogTurnCompleteRequest
                {
                    ...
                    Interpretation = "Processed through the interactive AgentFramework sample host.",
                    Model = _settings.ModelId,
                    ModelProvider = "openai",
    ...
    {
        Console.WriteLine("McpServer.AgentFramework sample host");
        Console.WriteLine("-----------------------------------");
    ...
    private void WriteHelp()
    {
        ...
        Console.WriteLine(@"  dotnet run --project src\McpServer.AgentFramework.SampleHost -c Debug -- ""List open TODO items""");
        Console.WriteLine(@"  dotnet run --project src\McpServer.AgentFramework.SampleHost -c Debug -- ""! Get-Location""");
        Console.WriteLine();
    }
