...
public sealed class ToolLifecycleSequenceTests
{
    ...
    public async Task FullToolLifecycle_CreateThroughDelete()
    {
        ...
        {
            // 1. List baseline
            _out.WriteLine("1. GET /mcp/tools — baseline");
            ...
            // 2. Create
            _out.WriteLine("2. POST /mcp/tools — create");
            var createBody = new
            {
            ...
            // 3. List +1
            _out.WriteLine("3. GET /mcp/tools — verify +1");
            ...
            // 4. Get by ID
            _out.WriteLine("4. GET /mcp/tools/{id}");
            ...
            // 5. Search by tag
            _out.WriteLine("5. GET /mcp/tools/search?keyword={tag}");
            ...
            // 6. Update
            _out.WriteLine("6. PUT /mcp/tools/{id} — update description");
            ...
            // 7. Get verify update
            _out.WriteLine("7. GET /mcp/tools/{id} — verify update");
    ...
    public async Task FullBucketLifecycle_AddThroughRemove()
    {
        ...
        {
            // 1. List baseline
            _out.WriteLine("1. GET /mcp/tools/buckets — baseline");
            var list = await _f.Client.GetFromJsonAsync<BucketListResult>(ToolRegistryFixture.BucketRoute);
            Assert.NotNull(list);
            var baseline = list.TotalCount;

            // 2. Add
            _out.WriteLine("2. POST /mcp/tools/buckets — add");
            ...
            // 3. List +1
            _out.WriteLine("3. GET /mcp/tools/buckets — verify +1");
            ...
            _out.WriteLine("4. POST /mcp/tools/buckets/{name}/sync");
            ...
            // 5. Browse
            _out.WriteLine("5. GET /mcp/tools/buckets/{name}/browse");
            ...
            // 6. Remove
            _out.WriteLine("6. DELETE /mcp/tools/buckets/{name}");
            ...
            // 7. List restored
            _out.WriteLine("7. GET /mcp/tools/buckets — verify count restored");
