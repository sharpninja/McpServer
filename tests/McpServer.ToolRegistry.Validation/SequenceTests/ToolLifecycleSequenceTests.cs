...
public sealed class ToolLifecycleSequenceTests
{
    ...
    public async Task FullToolLifecycle_CreateThroughDelete()
    {
        ...
        try
        {
            ...
            var list = await _f.Client.GetFromJsonAsync<ToolSearchResult>(ToolRegistryFixture.ToolRoute, cancellationToken: TestContext.Current.CancellationToken);
            Assert.NotNull(list);
            ...
            var create = await _f.Client.PostAsJsonAsync(ToolRegistryFixture.ToolRoute, createBody, cancellationToken: TestContext.Current.CancellationToken);
            ...
            var list2 = await _f.Client.GetFromJsonAsync<ToolSearchResult>(ToolRegistryFixture.ToolRoute, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(baseline + 1, list2!.TotalCount);

            // 4. Get by ID
            _out.WriteLine("4. GET /mcpserver/tools/{id}");
            var get = await _f.Client.GetFromJsonAsync<ToolDto>($"{ToolRegistryFixture.ToolRoute}/{toolId}", cancellationToken: TestContext.Current.CancellationToken);
            Assert.NotNull(get);
            ...
            var upd = await _f.Client.PutAsJsonAsync(
                ...
                new { Description = "Updated lifecycle tool" }, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.OK, upd.StatusCode);
            var updRes = await upd.Content.ReadFromJsonAsync<ToolMutationResult>(cancellationToken: TestContext.Current.CancellationToken);
            ...
            var get2 = await _f.Client.GetFromJsonAsync<ToolDto>($"{ToolRegistryFixture.ToolRoute}/{toolId}", cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal("Updated lifecycle tool", get2!.Description);

            // 8. Delete
            _out.WriteLine("8. DELETE /mcpserver/tools/{id}");
            var del = await _f.Client.DeleteAsync($"{ToolRegistryFixture.ToolRoute}/{toolId}", cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.OK, del.StatusCode);
            toolId = 0;

            // 9. Verify 404
            _out.WriteLine("9. GET /mcpserver/tools/{id} — verify 404");
            var gone = await _f.Client.GetAsync($"{ToolRegistryFixture.ToolRoute}/{createRes.Tool.Id}", cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.NotFound, gone.StatusCode);

            // 10. List restored
            _out.WriteLine("10. GET /mcpserver/tools — verify count restored");
            var listFinal = await _f.Client.GetFromJsonAsync<ToolSearchResult>(ToolRegistryFixture.ToolRoute, cancellationToken: TestContext.Current.CancellationToken);
        ...
        {
            if (toolId > 0) try { await _f.Client.DeleteAsync($"{ToolRegistryFixture.ToolRoute}/{toolId}", cancellationToken: TestContext.Current.CancellationToken); } catch { }
            throw;
        }
    }
    ...
    public async Task FullBucketLifecycle_AddThroughRemove()
    {
        ...
        try
        {
            ...
            var list = await _f.Client.GetFromJsonAsync<BucketListResult>(ToolRegistryFixture.BucketRoute, cancellationToken: TestContext.Current.CancellationToken);
            Assert.NotNull(list);
            var baseline = list.TotalCount;
            ...
            var add = await _f.Client.PostAsJsonAsync(ToolRegistryFixture.BucketRoute, body, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.Created, add.StatusCode);

            // 3. List +1
            _out.WriteLine("3. GET /mcpserver/tools/buckets — verify +1");
            var list2 = await _f.Client.GetFromJsonAsync<BucketListResult>(ToolRegistryFixture.BucketRoute, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(baseline + 1, list2!.TotalCount);

            // 4. Sync (200 if manifests exist, 404 if path has no manifests)
            _out.WriteLine("4. POST /mcpserver/tools/buckets/{name}/sync");
            var sync = await _f.Client.PostAsync($"{ToolRegistryFixture.BucketRoute}/{bucketName}/sync", null, cancellationToken: TestContext.Current.CancellationToken);
            Assert.True(
            ...
            var browse = await _f.Client.GetAsync($"{ToolRegistryFixture.BucketRoute}/{bucketName}/browse", cancellationToken: TestContext.Current.CancellationToken);
            ...
            var rem = await _f.Client.DeleteAsync($"{ToolRegistryFixture.BucketRoute}/{bucketName}", cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(HttpStatusCode.OK, rem.StatusCode);

            // 7. List restored
            _out.WriteLine("7. GET /mcpserver/tools/buckets — verify count restored");
            var listFinal = await _f.Client.GetFromJsonAsync<BucketListResult>(ToolRegistryFixture.BucketRoute, cancellationToken: TestContext.Current.CancellationToken);
            Assert.Equal(baseline, listFinal!.TotalCount);
        ...
        {
            try { await _f.Client.DeleteAsync($"{ToolRegistryFixture.BucketRoute}/{bucketName}", cancellationToken: TestContext.Current.CancellationToken); } catch { }
            throw;
        }
    }
}
