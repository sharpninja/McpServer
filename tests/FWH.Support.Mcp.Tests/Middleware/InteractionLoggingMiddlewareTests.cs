...
namespace FWH.Support.Mcp.Tests.Middleware;
...
{
    private static ILogger<FWH.Support.Mcp.Middleware.InteractionLoggingMiddleware> CreateLogger() =>
        Substitute.For<ILogger<FWH.Support.Mcp.Middleware.InteractionLoggingMiddleware>>();
    ...
    public async Task InvokeAsync_CallsNext_AndEnqueuesWhenUrlConfigured()
    {
        ...
        var middleware = new FWH.Support.Mcp.Middleware.InteractionLoggingMiddleware(next, logger, options, channel);
    ...
    public async Task InvokeAsync_Throws_WhenContextNull()
    {
        ...
        var logger = CreateLogger();
        var options = Microsoft.Extensions.Options.Options.Create(new McpInteractionLoggingOptions());
        var middleware = new FWH.Support.Mcp.Middleware.InteractionLoggingMiddleware(next, logger, options);
