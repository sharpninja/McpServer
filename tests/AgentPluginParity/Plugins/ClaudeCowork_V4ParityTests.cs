// Phase 9 parity adoption tests for mcpserver-claude-cowork-plugin (TR-MCP-AGENT-PARITY-020).
...
// Plugin root resolved from sibling directory or CLAUDE_COWORK_PLUGIN_ROOT env var.
using System;
using System.IO;
using Xunit;
...
/// <summary>TR-MCP-AGENT-PARITY-020: Phase 9 parity assertions for mcpserver-claude-cowork-plugin.</summary>
public class ClaudeCowork_V4ParityTests
{
    internal static string ResolvePluginRoot()
    {
        var repoRoot = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", ".."));
        var sibling = Path.GetFullPath(Path.Combine(repoRoot, "..", "mcpserver-claude-cowork-plugin"));
        if (Directory.Exists(sibling)) return sibling;

        var envRoot = Environment.GetEnvironmentVariable("CLAUDE_COWORK_PLUGIN_ROOT");
    ...
    [Fact]
    public void ClaudeCowork_CacheManager_MaxRetriesIsThree()
    {
    ...
    [Fact]
    public void ClaudeCowork_HooksJson_HasFullRequiredSurface()
    {
    ...
    [Fact]
    public void ClaudeCowork_Skills_HasAll5CoreSurfaces()
    {
    ...
    [Fact]
    public void ClaudeCowork_HasEnforcementMd_ReferencingSharedProtocol()
    {
        ...
        Assert.True(found != null, "ENFORCEMENT.md missing (required for Phase 9 parity)");
    ...
    [Fact]
    public void ClaudeCowork_CacheScope_ProvidesBase64UrlWorkspaceKey()
    {
    ...
    [Fact]
    public void ClaudeCowork_CacheScope_ProvidesV4FailsafeLayout()
    {
        var content = ReadLib("cache-scope.sh");
    ...
    [Fact]
    public void ClaudeCowork_SubagentCapture_ScriptsPresent()
    {
