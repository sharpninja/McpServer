...
public sealed class SessionLogWorkflowTests
{
    ...
    public async Task TurnLifecycle_AppendsDialogAndActionsThenCompletesAndPersists()
    {
        ...
        {
            QueryTitle = "Implement AgentFramework session-log workflow",
        ...
        await workflow.AppendActionsAsync(new SessionLogActionAppendRequest
        {
            ...
            [
                new UnifiedActionDto
                {
                    ...
                    Type = "edit",
                    Status = "completed",
                    FilePath = @"src\McpServer.AgentFramework\SessionLog\SessionLogWorkflow.cs",
                },
                new UnifiedActionDto
                {
                    ...
                    Type = "create",
                    Status = "completed",
                    FilePath = @"tests\McpServer.AgentFramework.Tests\SessionLogWorkflowTests.cs",
                },
            ],
        });
        ...
        var completedTurn = await workflow.CompleteTurnAsync(new SessionLogTurnCompleteRequest
        {
            ...
            FilesModified =
            [
                @"src\McpServer.AgentFramework\SessionLog\SessionLogWorkflow.cs",
                @"tests\McpServer.AgentFramework.Tests\SessionLogWorkflowTests.cs",
            ],
            DesignDecisions =
            [
        ...
        Assert.Equal("req-20260309T150105Z-implement-agentframework-session-log-workflow", turn.RequestId);
    ...
    private static (McpServerClient Client, SessionLogWorkflow Workflow, RecordingHttpMessageHandler Handler) CreateSut()
    {
        ...
        var identifiers = new McpSessionIdentifierFactory(
            Options.Create(new McpAgentFrameworkOptions
            {
                ApiKey = "test-key",
                BaseUrl = new Uri("http://localhost:7147"),
                SourceType = "Codex",
