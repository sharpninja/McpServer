...
public sealed class SessionLogSanitizerProjectionTests
{
    ...
    private static UnifiedSessionLogDto CreateSessionLog()
    {
        return new UnifiedSessionLogDto
        {
            ...
            AgentSessionId = $"agent-session password={Secret}",
            AgentSessionTranscriptFile = $"transcript password={Secret}",
            AgentExecutablePath = $"executable password={Secret}",
            AgentExecutableVersion = $"version password={Secret}",
